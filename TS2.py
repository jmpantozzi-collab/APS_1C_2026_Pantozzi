...
XXmod= np.abs(XX)
XXphase= np.angle(XX)
XXmod_ad=(XXmod**2)*2 #Ajustamos con factor de escala 2 porque tengo visualizado 1, por simetria la otra es igual
 #Escalon para evitar valores 0 que rompen el log

XXmod_dB = 10 * np.log10(XXmod_ad**2) 

plt.figure(2)
plt.clf()
plt.plot (XXmod_ad[:500], label ='S')
...
XXrmod_dB = 10 * np.log10((XXrmod / np.max(XXrmod))**2)
...
XXqmod_dB = 10 * np.log10((XXqmod/ np.max(XXqmod))**2)
...
plt.plot (XXmod_dB[:500], label = 'S', color = 'blue')
plt.plot (XXrmod_dB[:500], label = 'S+r', color = 'orange',linestyle='dotted')
plt.plot([piso_analog]*500, color='pink', linestyle='dashed', label='piso analogico')
